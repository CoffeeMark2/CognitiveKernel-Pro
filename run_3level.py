input_jsonl_file = 'result/gaia_20250905.jsonl'
...
import datetime
import pandas as pd
from collections import defaultdict # 引入一个方便的工具
...
def process_and_evaluate_jsonl_combined(
    ...
    level_successes = defaultdict(int)
    # 3. 主处理循环
    try:
        with open(file_path, 'r', encoding='utf-8') as f_in, \
            ...
            for line_num, line in enumerate(f_in, 1):
                ...
                try:
                    ...
                    level = data.get('_orig', {}).get('Level', 'Unknown') 
                    row_result['Level'] = level # 也把它加到结果里，方便在CSV里查看
        ...
        column_order = ['ID', 'Level', 'Result', 'Prediction', 'Gold']
        ...
        for level in sorted_levels:
            ...
            print(f"  - {level}: {rate:.2f}% ({successes}/{count})")
        
        print(f"\n  - 原文件 '{os.path.basename(file_path)}' 已更新，仅保留成功案例。")
        print(f"  - 详细评估报告已保存至 '{results_filename}'。")
        print("-"*60)
